...
Instruction *IRBuilder::visitTryBlock(parser::TryBlock *node) {
    ...
    if (!node->catches.empty()) {
        ...
        for (auto &entry: node->catches) {
            ...
            if (catch_block->err != nullptr) {
                // FIXME
                const auto *param = (parser::Identifier *) catch_block->err;

                auto *lexc = this->builder_.CreateUnaryOp(orbiter::OPCode::LDEXC);

                this->StoreVariable(param->symbol, lexc, true);
            }
