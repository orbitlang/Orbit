...
HOObject Loader::LoadFunction(NativeBinding *binding) const {
    const char *library = nullptr;
    if (binding->library != nullptr)
        library = ORSTRING_TO_CSTR(binding->library);

    const DLHandle lib = OpenLibrary(this->isolate_, library);
    if (lib == DLHandleError)
        return {};

    auto *func = LoadSymbol(this->isolate_, lib, ORSTRING_TO_CSTR(binding->symbol));
    if (func == DLHandleError) {
        CloseLibrary(this->isolate_, lib, library);
...
HOObject Loader::LoadVariable(NativeBinding *binding) const {
    assert(false);
...
HORString Loader::MakeKey(const ORString *library, const ORString *symbol, const NativeBindingType type) const {
    ...
    auto s_str = ORSTRING_TO_CSTR(symbol);
...
HOObject Loader::Load(NativeBinding *binding) const {
